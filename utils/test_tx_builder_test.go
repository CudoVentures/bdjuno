...
func TestTxBuilder_Build(t *testing.T) {
	timestamp := time.Now()
	tx, err := NewTestTx(timestamp).WithEventCreateGroup(num, str).WithEventSubmitProposal(num).WithEventExec(resultDefault).WithEventVote().WithEventWithdrawProposal().Build()
