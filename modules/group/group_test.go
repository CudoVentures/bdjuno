...
func (suite *GroupModuleTestSuite) SetupTest() {
	...
	suite.module = NewModule(params.MakeTestEncodingConfig().Codec, db)
	suite.db = db
...
func (suite *GroupModuleTestSuite) TestGroup_HandleMsgCreateGroupWithPolicy() {
	tx := mockutils.NewMockTxBuilder(suite.T(), timestamp, oneStr, one).WithEventCreateGroup(two, twoStr).Build()
...
func (suite *GroupModuleTestSuite) TestGroup_HandleMsgSubmitProposal() {
	tx := mockutils.NewMockTxBuilder(suite.T(), timestamp, oneStr, one).WithEventSubmitProposal(two).Build()
...
func (suite *GroupModuleTestSuite) TestGroup_HandleMsgSubmitProposal_TryExec() {
	...
	msg, err := group.NewMsgSubmitProposal(oneStr, []string{twoStr}, []types.Msg{}, twoStr, execTry, twoStr, twoStr)
	suite.Require().NoError(err)
...
func (suite *GroupModuleTestSuite) TestGroup_HandleMsgVote_UpdateStatusToAccepted() {
	tx := mockutils.NewMockTxBuilder(suite.T(), timestamp, oneStr, one).WithEventVote().Build()
...
func (suite *GroupModuleTestSuite) TestGroup_HandleMsgExec() {
	tx := mockutils.NewMockTxBuilder(suite.T(), timestamp, oneStr, one).WithEventExec(resultSuccess).Build()
...
func (suite *GroupModuleTestSuite) TestGroup_HandleMsgExec_HandleMsgUpdateGroup() {
	tx := mockutils.NewMockTxBuilder(suite.T(), timestamp, oneStr, one).WithEventSubmitProposal(two).WithEventVote().WithEventExec(resultSuccess).Build()
...
func (suite *GroupModuleTestSuite) TestGroup_HandleMsgWithdrawProposal() {
	tx := mockutils.NewMockTxBuilder(suite.T(), timestamp, oneStr, one).WithEventWithdrawProposal().Build()
