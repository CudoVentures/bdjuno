...
func (bmw blocksMonitoringWorker) Start(ctx context.Context, parseCfg *parsecmdtypes.Config, parseCtx *parser.Context, storage keyValueStorage, interval time.Duration) {
	bmw.baseWorker.Start(ctx, bmw.Name(), bmw.monitorBlocks, parseCfg, parseCtx, storage, interval)
}

func (bmw blocksMonitoringWorker) monitorBlocks(parseCfg *parsecmdtypes.Config, parseCtx *parser.Context, storage keyValueStorage) error {
