...
func (fbw fixBlocksWorker) Start(ctx context.Context, parseCfg *parsecmdtypes.Config, parseCtx *parser.Context, storage keyValueStorage, interval time.Duration) {
	fbw.baseWorker.Start(ctx, fbw.Name(), fbw.fixBlocks, parseCfg, parseCtx, storage, interval)
}

func (fbw fixBlocksWorker) fixBlocks(parseCfg *parsecmdtypes.Config, parseCtx *parser.Context, storage keyValueStorage) error {
