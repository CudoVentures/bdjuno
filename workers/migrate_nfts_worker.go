...
func (mnw migrateNftsWorker) Start(ctx context.Context, parseCfg *parsecmdtypes.Config, parseCtx *parser.Context, storage keyValueStorage, interval time.Duration) {
	mnw.baseWorker.Start(ctx, mnw.Name(), mnw.migrateNfts, parseCfg, parseCtx, storage, interval)
}

func (mnw migrateNftsWorker) migrateNfts(parseCfg *parsecmdtypes.Config, parseCtx *parser.Context, storage keyValueStorage) error {
	...
	nftModule := nft.NewModule(parseCtx.EncodingConfig.Codec, database.Cast(parseCtx.Database))
...
func (mnw migrateNftsWorker) processBlock(module *nft.Module, parseCtx *parser.Context, currentHeight int64) error {
	...
	for _, tx := range txs {
		...
		for _, msg := range tx.Body.Messages {
			var stdMsg sdk.Msg
			if err := parseCtx.EncodingConfig.Codec.UnpackAny(msg, &stdMsg); err != nil {
